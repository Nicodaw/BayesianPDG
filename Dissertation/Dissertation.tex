...
%TODO: Add benefit: unknown data can be handled implicitly by treating it as an unknown parameter and estimating it as well. Details on how parameters are estimated? EM.
...
\paragraph{}
The dungeon sizes in our dataset range from two to 27, but due to our restrictions combined with chance, dungeons of some sizes were never recorded. Namely dungeons with sizes \(\{4,23\}\). From a statistical point of view we can conclude that lack of dungeons from those sizes makes them highly improbable to the point where we should not be concerned with their existence. This is what we do with dungeons that are outside of the range of sizes for one-page adventures. But in terms of utility of our tool we would like to be able generate any dungeon within that range. Therefore we will linearly interpolate \textit{LI} dungeons that are \textit{neighbouring} the missing ones in order for us to get estimates for the global parameters. We have chosen LI due to its simplicity and the fact it has been shown to produce good results in general \parencite{Ibargengoytia2013OnTE} and exceptional results on small datasets \parencite{yu2004advances}. %TODO: describe better why we don't like linear interpolation here
Room parameters could potentially also be linearly interpolated by using all the rooms from the neighbouring dungeons but this poses several questions about what does it mean to interpolate 5 rooms and 6 rooms. A better and more intuitive way to handle them is by leveraging the properties of the BNets. We have interpolated the global parameters which means that we assert that dungeons with those sizes and critical paths exist within the universe of possible dungeons and we can further infer the related room parameters by treating them as unknown values. In that way, when we're learning the parameters we would also get estimates for our unknown rooms.
%TODO:Linear interpolation
%TODO:Consider missing rooms as unobserved parameters that we are also estimating
...
\begin{table}[htb]
  ...
  \begin{tabular}{@{}llllllllllll@{}}
    \toprule
                    & \multicolumn{2}{l}{\texttt{NumRooms}} & \multicolumn{2}{l}{\texttt{CPLength}} & \multicolumn{2}{l}{\texttt{CPDistance}} & \multicolumn{2}{l}{\texttt{Depth}} & \multicolumn{2}{l}{\texttt{Neighbours}}&                        \\ \midrule
                    & \(loss^2\)   & \(E\)         & \(loss^2\)   & \(E\)         & \(loss^2\)    & \(E\)          & \(loss^2\)  & \(E\)       & \(loss^2\)    & \(E\)          &                        \\
    Uniform (base)  & 0.9615       & 99.83\%       & 0.9          & 97.52\%       & 0.8333        & 57.86\%        & 0.9286      & 91.97\%     & 0.8889        & 70.51\%        &                        \\ \midrule
    Extreme Sparse  & 0.7819       & 67.09\%       & 0.5294       & 41.79\%       & 0.6173        & 51.54\%        & 0.7657      & 61.54\%     & 0.7174        & 59.15\%        & \multirow{3}{*}{count} \\
    TAN             & 0.6871       & 54.87\%       & 0.5294       & 41.79\%       & 0.5812        & 41.62\%        & 0.6581      & 50.51\%     & 0.7054        & 49.74\%        &                        \\
    Fully Connected & 0.6012       & 44.53\%       & 0.3391       & 22.14\%       & 0.4205        & 17.44\%        & 0.5814      & 35.98\%     & 0.7029        & 25.64\%        &                        \\ \midrule
    Extreme Sparse  & 0.7522       & 66.67\%       & 0.4974       & 41.2\%        & 0.6237        & 54.19\%        & 0.7354      & 63.25\%     & 0.693         & 59.15\%        & \multirow{3}{*}{EM}    \\
    TAN             & 0.6081       & 50.77\%       & 0.4974       & 41.2\%        & 0.4968        & 41.62\%        & 0.562       & 45.73\%     & 0.6024        & 49.74\%        &                        \\
    Fully Connected &\textbf{0.4297}&\textbf{39.23\%}&\textbf{0.2097}& \textbf{18.72\%}& \textbf{0.1909}& \textbf{16.92\%}       & \textbf{0.3683}&\textbf{33.76\%}& \textbf{0.284}& \textbf{25.64\%}        &                        \\ \midrule
    Extreme Sparse  & 1.07         & 75.98\%       & 0.7947       & 50.34\%       & 0.6644        & 57.26\%        & 0.7951      & 66.75\%     & 0.7368        & 62.65\%        & \multirow{3}{*}{GD}    \\
    TAN             & 1.036        & 64.19\%       & 0.8043       & 52.31\%       & 0.5632        & 45.38\%        & 0.7532      & 54.19\%     & 0.735         & 55.73\%        &                        \\
    Fully Connected & 1.046        & 69.57\%       & 0.701        & 44.7\%        & 0.4416        & 38.8\%         & 0.6256      & 53.25\%     & 0.5295        & 45.38\%        &                        \\ \midrule
    \end{tabular}%
  }
...
The participants were selected to be people with any level of experience in PnP RPGs. 
%TODO: Who did the study?
