...
Similarly to structure learning from data, when we learn parameters (CPTs for each RV) we are trying to find the most likely PD for each RV that agrees with our data. More formally if \(\mathcal{N}\) is the network and \(\mathcal{D}\) is the data then from Bayes's rule we know that \(P(\mathcal{N} \mid \mathcal{D}) = \frac{P(\mathcal{D} \mid \mathcal{N})P(\mathcal{N})}{P(\mathcal{D})}\). We can formulate the parameter learning as an optimisation problem where we maximise the posterior \(P(\mathcal{D} \mid \mathcal{N})P(\mathcal{N})\) \parencite[46-48]{neticaCman}. Netica has three algorithms (Count, Expectation Maximisation and Gradient Descent) in its toolkit for achieving this. For completeness, it should be noted that all algorithms in Netica follow \textit{true Bayesian learning} principles 
