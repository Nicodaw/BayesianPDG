...
The ultimate validation for a successful PDG tool is undoubtedly a user study assessing how players use the generator for their games. Unfortunately as discussed in section \ref{subsub:BSP}, the nature of PnP RPGs makes this task very difficult to gather a representative sample size within the timeframe of the project. For this reason we will attempt answering the question of "What makes a compelling dungeon" by proxy. That is, in section \ref{sec:Aim} we have asserted that the ultimate dungeon generator capable of producing compelling dungeons is the human designer. Therefore we conduct a study in which we assess if our produced dungeon layouts are undistinguishable from human-made dungeons. Inspiration for this experiment has been drawn from previous work on the aesthetics of composition in which it has been shown that people can discriminate between computer generated and original artwork \parencite{McManusMondrian} \parencite{FurnhamMondrian}. To validate if our model is able to generate dungeons that are undistinguishable from human-made dungeons we will conduct a preference study. Dungeons from the OPDC, our inferred set and a random control group are selected and formatted in the same visual style. Three sets of pairs are randomly permuted and contain an even distribution of the possible combinations \textit{(OPDC,ours),(OPDC,random),(ours,random)}.

The common 10, 13 and 8 room dungeons as highlighted in section \ref{cha:results} were used and examples can be found in Appendix B fig \ref{fig:gen_dungeons}. The experiment was done as an online survey.
