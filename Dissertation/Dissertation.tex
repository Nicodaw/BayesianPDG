...
Bayesian Networks \textit{BNs} \parencite{pearl1985bayesian} (also referred to as Inference Networks, Belief Networks, Directed Acyclical Graph (DAG) Models and others) assumes our domain is defined in terms of a joint probability distribution \(P(X_1, \ldots , X_n)\) over a set of random variables \textit{RVs} \(X\), each of which has a set of potential values it can take \(X_i(\Omega) = \{x_1, \ldots, x_j\}\) with an associated probability (example: \(P(X_i = x_j) = \frac{1}{2}\) ). Each vertex in this graph is an RV whose edges indicate our belief of causality. % say something about the conditioning set and inferring

To contextualise this, we can look into Summerville's transcription of the Legend of Zelda dungeons \parencite{SummervilleLearningOfZelda,SummervilleSamplingHyrule}. After annotating the dungeon maps with elements of both Mission and Space (total number of rooms, treasures, monsters, critical path CP, etc.), he extracts features for the BN by parsing them as RVs. Given our data, if we condition on total number of rooms (i.e. \textit{observe} that RV), the probability distribution for the possible CP would reflect our new knowledge. Then by sampling the RV for the CP length we can fix the parameter and get a point estimate for critical path length given that number of rooms in the dungeon. This inference process can take various degrees of complexity 
...
The structure BNs provide allows us to create a natural description

\paragraph{What we need to do to use Bayesian Networks?}
In order for us to create an inference network that reflects our beliefs we need to define two key attributes of the BN:
\begin{outline}[enumerate]
...
% What problems can we encounter? Sparsity of data. Very different dungeons some 
\subsection{Extracting Features} % transforming the data to something useful for the BNet
% Is the data ready to be used in BNets? No, it needs to be transformed from a textual description to 

\paragraph{}
The final dataset consists of \textbf{[[size]]} organic entries and \textbf{[[synthetic]]} linearly interpolated entries for a total of \textbf{[[dungeon params]]} and \textbf{[[room params]]}. %ToDo: insert statistical figures about the dataset. Histograms of the different features, mean, variance and lower/upper bounds.
...
\subsubsection{TAN}
% We will also just in case compare the simplified DAG to a learned DAG structure. Explain how it works
