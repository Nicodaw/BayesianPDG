...
\end{outline}
\textbf{[[TODO: Explain this more clearly]]}
...
\begin{outline}[enumerate]
  \1 Data
    \2 availability and extraction. 
      \3 Could extend to include even non-winning entries for the sake of having a greater sample size. Winning entries are considered as a rule-of-thumb \textit{good} dungeons, but that does not imply that non-winning entries are examples of \textit{bad} dungeons. \textbf{[[TODO: Discussion about non-binary quality assessment and how we can think about the trade-off between quality and sample size]]} . More datasets can be considered. We've use OPDC due to its CC license, but paid modules exist.
...
%TODO: A summarising statement
% conceptually data-driven methods for tabletop RPGs would 
%and although our Space-only model has unsatisfactory performance, we outlined a pipeline that allows for rigorous exploratory analysis of different methods for data-driven PDG
% Identified a lot of practical problems with data-driven methodologies for PDGs
